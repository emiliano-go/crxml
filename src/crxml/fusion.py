...
def _arrow_iter(table) -> Iterator[dict]:
    """Yield dicts from a pyarrow Table (compat helper)."""
    for i in range(table.num_rows):
        yield {col: table.column(col)[i].as_py() for col in table.column_names}


def _try_columnar_fusion(source, stages):
    """Merge fusible stages into the source's columnar BuildPlan.

    Returns an iterator over the fused result, or *None* when no stage
    can be fused into the plan (in which case the caller should fall
    back to the regular dict path).
    """
...
def fused_iter(source: Iterable[dict], stages: list[Callable]) -> Iterator[dict]:
    # Try columnar plan fusion first.
    result = _try_columnar_fusion(source, stages)
    if result is not None:
        return result

    # Fallback: regular dict-path fusion.
    fusables = []
    rem = list(stages)
